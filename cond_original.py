...
class UnetUp(nn.Module):
    def __init__(self, in_channels, out_channels, padding=0):
        ...
        '''
        layers = [
            nn.ConvTranspose2d(in_channels, out_channels, 2, 2, padding=padding),
...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat = 256, n_classes=3):
        ...
        self.contextembed3 = EmbedFC(n_classes, 2*n_feat)
        self.contextembed4 = EmbedFC(n_classes, 1*n_feat)

        self.up0 = nn.Sequential(
            # nn.ConvTranspose2d(6 * n_feat, 2 * n_feat, 7, 7), # when concat temb and cemb end up w 6*n_feat
            nn.ConvTranspose2d(8 * n_feat, 8 * n_feat, 5, 5), # otherwise just have 2*n_feat
            nn.GroupNorm(8, 8 * n_feat),
            nn.ReLU(),
        )

        self.up1 = UnetUp(16 * n_feat, 4 * n_feat, padding=0)
        self.up2 = UnetUp(8 * n_feat, 2 * n_feat, padding=0)
    ...
    def forward(self, x, cxt, t, context_mask):
        ...
        cxt = nn.functional.one_hot(cxt, num_classes=self.n_classes).type(torch.float)
        
        # mask out context if context_mask == 1
        context_mask = context_mask[:, None]
        context_mask = context_mask.repeat(1,self.n_classes)
        context_mask = (-1*(1-context_mask)) # need to flip 0 <-> 1
        cxt = cxt * context_mask
        
        # embed context, time step
        cemb1 = self.contextembed1(cxt).view(-1, self.n_feat * 8, 1, 1)
        temb1 = self.timeembed1(t).view(-1, self.n_feat * 8, 1, 1)
        cemb2 = self.contextembed2(cxt).view(-1, self.n_feat * 4, 1, 1)
        temb2 = self.timeembed2(t).view(-1, self.n_feat * 4, 1, 1)
        cemb3 = self.contextembed3(cxt).view(-1, self.n_feat * 2, 1, 1)
        temb3 = self.timeembed3(t).view(-1, self.n_feat * 2, 1, 1)
        cemb4 = self.contextembed4(cxt).view(-1, self.n_feat, 1, 1)
        ...
        up4 = self.up3(cemb3*up3+ temb3, down2[..., :20, :20])
        import ipdb; ipdb.set_trace()
        up5 = self.up4(cemb4*up4+ temb4, down1[..., :40, :40])
...
def train_mnist():
    ...
    # dataset = torch.load("frame_dataset.pt")
    # dataset = torch.load("frame_dataset_90.pt")
    dataset = torch.load("frame_dataset_84.pt")
